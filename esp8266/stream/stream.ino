...
void setup() {
  ...
  outbufferFront = (uint8_t*)malloc(360 * 16);
  outbufferBack = (uint8_t*)malloc(360 * 16);
  for (int i = 0; i < 360 * 16; i++)
    outbufferFront[i] = 0;
...
ICACHE_RAM_ATTR void espRdFall() {
  ...
  } while(outFrontIndex & 0x000f); //We have sent one tile if the index is a multiple of 16 again.
...
void handleDataFromTcp() {
  ...
  while (client.available() >= 16) {
    client.read(outbufferBack + outBackIndex, 16);
    outBackIndex += 16;
    if (outBackIndex >= 360 * 16) {
      outBackIndex = 0;
      outBackFilled = true;
      return;
    }
